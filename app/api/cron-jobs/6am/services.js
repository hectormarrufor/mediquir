...
import db, {
    ...
    Activo, DocumentoActivo, BcvPrecioHistorico, HorasTrabajadas, Flete, Vehiculo, Remolque,
    Requisicion,
...
// ==========================================
// 5. REGISTRO AUTOMÁTICO DE HORAS Y ESTADOS (CRON 6AM)
// ==========================================
export async function checkFletesActivosYRegistrarHoras() {
    const today = getCaracasDate();
    const fechaActualStr = new Intl.DateTimeFormat('en-CA', {
        timeZone: 'America/Caracas', year: 'numeric', month: '2-digit', day: '2-digit',
    }).format(today);
    ...
    // 🔥 PASO 1: ACTIVAR FLETES PROGRAMADOS QUE INICIAN HOY
    const activables = await Flete.update(
        { estado: 'en_ruta' },
        { 
            where: { 
                estado: 'programado', 
                fechaSalida: { [db.Sequelize.Op.lte]: today } 
            } 
        }
    );
    activadosHoy = activables[0] || 0;

    // 🔥 PASO 2: BUSCAR FLETES EN RUTA PARA PAGARLES
    const fletesActivos = await Flete.findAll({
        where: { estado: 'en_ruta' },
        attributes: ['id', 'nroFlete', 'descripcion', 'chutos', 'remolques', 'breakdown'],
    });

    if (fletesActivos.length === 0) {
        return { msg: `Se activaron ${activadosHoy}. No hay fletes operando hoy.`, count: 0 };
    }

    const nuevosRegistros = [];
    const fletesParaCompletar = [];

    // Iterar la flota en calle
    for (const flete of fletesActivos) {
        // A. Pagar las horas del día de hoy
        const chutos = flete.chutos || []; 
        
        for (const chuto of chutos) {
            let nombreVehiculo = "Vehículo No Identificado";
            
            if (chuto.activoId) {
                const dbActivo = await Activo.findByPk(chuto.activoId, {
                    attributes: ['id', 'codigoInterno', 'tipoActivo'],
                    include: [
                        { association: 'vehiculoInstancia', attributes: ['placa'], include: [{ association: "plantilla", attributes: ['modelo', 'tipoVehiculo'] }] },
                        { association: 'remolqueInstancia', attributes: ['placa'], include: [{ association: "plantilla", attributes: ['modelo', 'tipoRemolque'] }] },
                        { association: 'maquinaInstancia', attributes: ['placa'], include: [{ association: "plantilla", attributes: ['modelo', 'tipo'] }] },
                    ]
                });

                if (dbActivo) {
                    if (dbActivo.tipoActivo === 'Vehiculo' && dbActivo.vehiculoInstancia) {
                        const plantilla = dbActivo.vehiculoInstancia.plantilla;
                        nombreVehiculo = `${dbActivo.codigoInterno} - ${plantilla?.tipoVehiculo || 'Vehículo'} ${plantilla?.modelo || ''} (Placa: ${dbActivo.vehiculoInstancia.placa || 'S/P'})`;
                    } else if (dbActivo.tipoActivo === 'Remolque' && dbActivo.remolqueInstancia) {
                        const plantilla = dbActivo.remolqueInstancia.plantilla;
                        nombreVehiculo = `${dbActivo.codigoInterno} - ${plantilla?.tipoRemolque || 'Remolque'} ${plantilla?.modelo || ''} (Placa: ${dbActivo.remolqueInstancia.placa || 'S/P'})`;
                    } else {
                        nombreVehiculo = dbActivo.codigoInterno; 
                    }
                }
            }

            const observacionBase = `Operación en Ruta: ${flete.nroFlete} - ${flete.descripcion || 'Sin descripción'}. Unidad: ${nombreVehiculo}`;

            const personalIds = [];
            if (chuto.choferId) personalIds.push(chuto.choferId);
            if (chuto.ayudantesIds) {
                let ayArray = Array.isArray(chuto.ayudantesIds) ? chuto.ayudantesIds : [chuto.ayudantesIds];
                ayArray.forEach(ay => {
                    const idStr = typeof ay === 'object' ? String(ay.id || ay.value || ay) : String(ay);
                    if (idStr && idStr.trim() !== '') personalIds.push(idStr);
                });
            }

            personalIds.forEach(empleadoId => {
        ...
        // B. Auditar si la fecha de llegada ya caducó (para cerrarlo automáticamente)
        if (flete.breakdown && flete.breakdown.rutinaViaje && flete.breakdown.rutinaViaje.fechaLlegadaISO) {
            const llegadaTime = new Date(flete.breakdown.rutinaViaje.fechaLlegadaISO).getTime();
            if (nowTimestamp >= llegadaTime) {
                fletesParaCompletar.push(flete.id);
            }
        }
    }

    // Inserción en Base de Datos (Protección anti-duplicados)
    if (nuevosRegistros.length > 0) {
        for (const registro of nuevosRegistros) {
            const existe = await HorasTrabajadas.findOne({
                where: { 
                    empleadoId: registro.empleadoId, 
                    fecha: registro.fecha,
                    origen: 'odt', 
                    observaciones: { [db.Sequelize.Op.like]: `%${registro.observaciones.split('-')[0].trim()}%` }
                }
            });
    ...
    // 🔥 PASO 3: FINALIZAR FLETES VENCIDOS
    if (fletesParaCompletar.length > 0) {
        await Flete.update(
            { estado: 'completado', fechaRetorno: new Date() },
            { where: { id: { [db.Sequelize.Op.in]: fletesParaCompletar } } }
        );
        finalizadosHoy = fletesParaCompletar.length;
    }

    return { 
        msg: `Flota Activa: ${fletesActivos.length}. Se pagaron ${registrosCreados} días. Activados: ${activadosHoy}, Finalizados: ${finalizadosHoy}.`, 
        count: registrosCreados 
    };
}


// ==========================================
// 6. REVISION DE REQUISICIONES POR APROBAR X EL JEFE (PRESIDENTE)
// ==========================================
export async function checkRequisicionesPorAprobar() {
    try {
        const requisiciones = await Requisicion.findAll({
            where: {
                estado: 'Pendiente'
            },
            attributes: ['id', 'codigo', 'prioridad', 'justificacion', 'fechaSolicitud'],
            include: [
                { 
                ...
                },
                {
                    ...
                    attributes: ['cantidadSolicitada'],
                    include: [
                        { association: 'consumible', attributes: ['nombre', 'unidadMedida'] }
                    ]
                },
                {
                    association: 'ordenOrigen',
                    attributes: ['codigo'],
                    include: [
                        {
                            association: 'activo',
                ...
                // 🔥 TRAEMOS EL HALLAZGO ORIGEN (EL PROBLEMA A RESOLVER) 🔥
                {
                    association: 'hallazgoOrigen',
        ...
        console.log(`Requisiciones pendientes encontradas: ${requisiciones.length}`);

        return requisiciones.map(req => {
            // 1. Quién pide
            const nombreSolicitante = req.solicitante?.empleado 
                ? `${req.solicitante.empleado.nombre} ${req.solicitante.empleado.apellido}` 
                : 'Admin';

            // 2. Equipo afectado
            let infoEquipo = 'Stock General';
            if (req.ordenOrigen?.activo) {
                const activo = req.ordenOrigen.activo;
                const { vehiculoInstancia, remolqueInstancia, maquinaInstancia } = activo;
                
                if (vehiculoInstancia?.plantilla) infoEquipo = `${vehiculoInstancia.plantilla.marca} ${vehiculoInstancia.plantilla.modelo} [${activo.codigoInterno}]`;
                else if (remolqueInstancia?.plantilla) infoEquipo = `${remolqueInstancia.plantilla.marca} ${remolqueInstancia.plantilla.modelo} [${activo.codigoInterno}]`;
                else if (maquinaInstancia?.plantilla) infoEquipo = `${maquinaInstancia.plantilla.marca} ${maquinaInstancia.plantilla.modelo}`;
                else infoEquipo = `Activo: ${activo.codigoInterno}`;
                
                infoEquipo = `${infoEquipo} (OM: ${req.ordenOrigen.codigo})`;
            }

            // 3. Resumen de items
            let resumenItems = 'Sin items especificados';
            if (req.detalles && req.detalles.length > 0) {
                const primerosItems = req.detalles.slice(0, 2).map(d => 
                    `${Number(d.cantidadSolicitada)} ${d.consumible?.unidadMedida} de ${d.consumible?.nombre}`
                ).join(', ');
                
                resumenItems = req.detalles.length > 2 
                    ? `${primerosItems} y ${req.detalles.length - 2} items más.` 
                    : primerosItems;
            }

            // 🔥 4. MOTIVO / PROBLEMA A RESOLVER 🔥
            let motivoResolucion = '';
            if (req.hallazgoOrigen?.descripcion) {
                motivoResolucion = `\nProblema a resolver: "${req.hallazgoOrigen.descripcion}"`;
            } else if (req.justificacion) {
                // Fallback por si es una requisición general de stock que no viene de un hallazgo
                motivoResolucion = `\nJustificación: ${req.justificacion}`;
            }
            ...
            return {
                ...
                // 🔥 ENSAMBLAMOS EL MENSAJE FINAL 🔥
                msg: `Se requieren: ${resumenItems}\nDestino: ${infoEquipo}${motivoResolucion}\nPedido por: ${nombreSolicitante}`
            };
        });

    } catch (error) {
        console.error("Error al revisar requisiciones por aprobar:", error);
        return [];
    }
}

// ==========================================
// 🔥 7. NUEVA AUDITORÍA: FIRMAS DE CUSTODIA PENDIENTES 🔥
// ==========================================
export async function checkPendingCustodySignatures() {
    try {
        // Límite de tiempo estricto: hace más de 24 horas atrás
        const limitHours = new Date(Date.now() - 24 * 60 * 60 * 1000);

        const morosas = await db.EntregaCustodia.findAll({
            where: {
                estado: 'Pendiente',
                createdAt: { [db.Sequelize.Op.lt]: limitHours }
            },
            include: [
                { association: 'receptor', attributes: ['nombre', 'apellido'] },
                { association: 'requisicion', attributes: ['codigo'] }
            ]
        });

        return morosas.map(item => ({
            id: item.id,
            entregadoPorId: item.entregadoPorId,
            receptorNombre: item.receptor ? `${item.receptor.nombre} ${item.receptor.apellido}` : 'Empleado',
            requisicionCodigo: item.requisicion?.codigo || 'N/A'
        }));
    } catch (error) {
        console.error("Error en servicio checkPendingCustodySignatures:", error);
        return [];
    }
}