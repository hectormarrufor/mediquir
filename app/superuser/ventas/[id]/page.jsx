        ...
        <Box p="md" maw={1400} mx="auto">
            <Group justify="space-between" mb="lg">
                <Group>
                    ...
                    <Badge size="lg" color={pedido.statusPago === 'Pagado' ? 'green' : 'orange'}>{pedido.statusPago}</Badge>
                </Group>
            </Group>
                                    ...
                                    <Table.Tbody>
                                        {pedido.detalles?.map(d => (
                                            <Table.Tr key={d.id}>
                                                ...
                                                <Table.Td>{d.isFicticio ? d.nombreFicticio : d.producto?.nombre}</Table.Td>
                        ...
                        <Grid.Col span={{ base: 12, md: 4 }}>
                            <Stack gap="md">
                                ...
                                <Paper withBorder p="md" radius="md" bg="gray.0">
                                    ...
                                    <Text size="sm" fw={600} mb={6}>{pedido.registrador?.empleado ? `${pedido.registrador.empleado.nombre} ${pedido.registrador.empleado.apellido}` : 'Admin'}</Text>
                                    
                                    <Text size="xs" c="dimmed">Empacador:</Text>
                                    <Text size="sm" fw={600} mb={6}>{pedido.empacador?.nombre ? `${pedido.empacador.nombre} ${pedido.empacador.apellido}` : 'Pendiente'}</Text>

                                    <Text size="xs" c="dimmed">Etiquetador:</Text>
                                    <Text size="sm" fw={600}>{pedido.etiquetador?.nombre ? `${pedido.etiquetador.nombre} ${pedido.etiquetador.apellido}` : 'Pendiente'}</Text>
                                </Paper>

                                <Paper withBorder p="md" radius="md" bg="gray.0">
                                    <Text fw={700} mb="xs" c="blue.9">Logística de Despacho</Text>
                                    <Text size="xs" c="dimmed">Chofer / Transporte:</Text>
                                    <Text size="sm" fw={600} mb={6}>{pedido.quienRetira || 'Pendiente'}</Text>
            ...
            <Modal opened={modalDespacho} onClose={() => setModalDespacho(false)} title={<Title order={4} c="grape">Despacho y Flete</Title>} centered>
                <form onSubmit={formDespacho.onSubmit(handleConfirmarDespacho)}>
                    <Stack gap="md">
                        <TextInput label="Chofer / Transporte" withAsterisk {...formDespacho.getInputProps('quienRetira')} />
                        <TextInput type="datetime-local" label="Fecha y Hora de Retiro" withAsterisk {...formDespacho.getInputProps('fechaHoraRetiro')} />
                        <NumberInput label="Costo del Flete (Gasto)" decimalScale={2} withAsterisk {...formDespacho.getInputProps('costoFlete')} />
                        <Button loading={isSubmitting} type="submit" fullWidth color="grape">Asentar Despacho y Gasto</Button>
                    </Stack>
                </form>
            </Modal>
